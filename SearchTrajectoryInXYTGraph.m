function [x, y, theta] = SearchTrajectoryInXYTGraph(bv)
...
[ind_vec, theta] = SearchViaAStar(start_ind, goal_ind, bv.theta0);
len = size(ind_vec,1);
...
function [ind_vec, theta] = SearchViaAStar(start_ind, goal_ind, theta0)
...
init_node(5) = 0;
init_node(6) = xyt_graph_search_.multiplier_for_H * sum(abs(start_ind(1:2) - goal_ind(1:2))) + xyt_graph_search_.weight_for_time * abs(start_ind(3) - goal_ind(3));
...
length_type_3 = abs(xyt_graph_search_.weight_for_time) + xyt_graph_search_.pelnaty_for_halt;
expansion_length = [
    length_type_1;
    length_type_2;
    length_type_1;
...
while ((~isempty(openlist_))&&(iter <= xyt_graph_search_.max_cycle)&&(~completeness_flag))
    ...
    for ii = 1 : size(expansion_pattern,1)
        ...
        child_g = cur_g + expansion_length(ii,1) + 0.25 * sum(abs(expansion_pattern(ii,1:2) - cur_operation(1:2))); % 0.25
        child_h = xyt_graph_search_.multiplier_for_H * (sum(abs(child_node_ind(1:2) - goal_ind(1:2))) + xyt_graph_search_.weight_for_time * abs(child_node_ind(3) - goal_ind(3)));
